    ...
    Object.keys(jsonData).forEach((_Type) => {
        Object.keys(jsonData[_Type]).forEach((_Category) => {
            ...
            Object.keys(jsonData[_Type][_Category]).forEach((_subCategory) => {
                subcategories.push(new SubCategory(_subCategory, jsonData[_Type][_Category][_subCategory]));
            });
...
function getInfo(categories: Category[], subCategory: string): Category[] {
    ...
    categories.forEach((category) => {
        const sub = category.getSubcategories().find((sub) => sub.Name.toLowerCase() === subCategory.toLowerCase());
...
const DataValidation: React.FC<DataValidationProps> = ({ categories }) => {
    ...
    return (
        <div className="categories">
            <IonItem>
                <IonInput placeholder="Enter a subcategory" onIonChange={(e) => setInput(e.detail.value!)}/>
            </IonItem>
            <IonButton className="ion-padding" onClick={() => setValidCategories(getInfo(categories, input))}>Get Info</IonButton>
            ...
            <div className="ion-padding" key={type}>
                ...
                {validCategories.filter((category) => category.getType() === type).map((category) => (
                <div key={category.getCategory()}>
                    <h4>{category.getCategory()}</h4>
                        ...
                        <p key={subCategory.Name}>
                            {subCategory.Name} - {subCategory.isStaticCategory() ? 'Static' : 'Dynamic'}
                        </p>
                        ))}
                </div>
                ))}
                ...
                <IonAccordion value={type} key={type}>
                    ...
                    <div slot="content" key={type}>
                        <IonAccordionGroup>
                            ...
                            {categories.filter((cat) => cat.getType() === type).map((category) => (
                            <IonAccordion value={category.getCategory()} key={category.getCategory()}>
                                ...
                                {/* Display the subcategories */}
                                {category.getSubcategories().map((subCategory) => (
                                <div slot="content" key={`${category.getType()}-${category.getCategory()}-${subCategory.Name}`}>
                                    <IonItem key={subCategory.Name}>
                                        <IonButton fill="clear" shape="round" onClick={() => console.log(category.getType() + ":\n\t", category.getCategory() + ":\n\t\t", subCategory.Name)}>
